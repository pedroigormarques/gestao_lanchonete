...
class PedidoFechadoBloc extends Bloc<PedidoFechadoEvent, PedidoFechadoState> {
  ...
  Future<void> _carregarListaPedidosFechados(
      CarregarListaPedidos event, Emitter<PedidoFechadoState> emit) async {
    emit(CarregandoPedidosFechados());
    try {
      ListaPedidosFechados listaPedidosFechados =
          await PedidosApiProvider.helper.getPedidosFechadosList();
      List<PedidoFechado> lista =
          listaPedidosFechados.carregarListaPedidosFechados();
      _calcularValores(lista);
      emit(PedidosFechadosCarregados(lista, _valorTotal,
          _produtosCardapioVendidos, _produtosEstoqueUsados));
    } catch (_) {
      emit(ErroCarregarPedidosFechados());
    }
  }

  void _atualizarListaComFiltro(
      CarregarListaPedidosFiltrados event, Emitter<PedidoFechadoState> emit) {
    ...
    if (state is PedidosFechadosCarregados) {
      ...
      if (event.periodo == null) {
        ...
        listaFiltrada = state.listaPedidosOriginal
            .where((pedido) =>
                pedido.horaAbertura.isAfter(event.periodo!.start) &&
                pedido.horaAbertura.isBefore(event.periodo!.end))
            .toList();
      }
      _calcularValores(listaFiltrada);
      emit(PedidosFechadosCarregados(state.listaPedidosOriginal, _valorTotal,
          _produtosCardapioVendidos, _produtosEstoqueUsados));
    } else {
      emit(ErroCarregarPedidosFechados());
    }
  ...
  void _calcularValores(List<PedidoFechado> lista) {
    ...
    _produtosCardapioVendidos = Map.fromEntries(
        _produtosCardapioVendidos.entries.toList()
          ..sort((e1, e2) => e2.value.compareTo(e1.value)));
    //ordenação alfabética
    _produtosEstoqueUsados = Map.fromEntries(
        _produtosEstoqueUsados.entries.toList()
